 ...
 *
 * If the file is a DataBook (.databook.md or parseable with frontmatter),
 * the rawBody is extracted so the template's own frontmatter is not included
 * in the output document.  Plain .md files are returned as-is.
 *
 * @param {string} filePath - Absolute path to template file.
 * @returns {string}        - Template body text.
 */
function loadTemplateBody(filePath) {
  if (!existsSync(filePath)) throw new Error(`template file not found: ${filePath}`);
  const raw = readFileSync(filePath, 'utf8');

  // Attempt DataBook parse — if it succeeds, use rawBody only.
  try {
    const db = parseDataBook(raw, filePath);
    if (db && db.rawBody) return db.rawBody;
  } catch { /* not a DataBook — fall through */ }

  return raw;
}

/**
 * Try to load the bundled default template.
 * Falls back silently to the inline builder if the file is missing or unreadable.
 *
 * @param {object[]} processedBlocks
 * @param {object}   frontmatter
 * @param {boolean}  quiet
 * @returns {string}
 */
function loadDefaultTemplateBody(processedBlocks, frontmatter, quiet) {
  try {
    return loadTemplateBody(DEFAULT_TEMPLATE_PATH);
  } catch (e) {
    if (!quiet) {
      process.stderr.write(
        `warn: W_DEFAULT_TEMPLATE_MISSING: bundled default template not found ` +
        `(${DEFAULT_TEMPLATE_PATH}); using inline builder.\n`
      );
    }
    return buildDefaultTemplate(processedBlocks, frontmatter);
  }
}
