 ...
 *      consecutively. No blank lines permitted between annotations and the fence.
 ...
 *      before the opening fence (canonical v1.2+ placement). No blank lines
 *      permitted between the annotations and the fence.
...
export function parseBlocks(bodyLines, frontmatter = null) {
  ...
  while (i < bodyLines.length) {
    ...
    while (lb >= 0) {
      const m = RE_META_COMMENT.exec(bodyLines[lb]);
      if (m) {
