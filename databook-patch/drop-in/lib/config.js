...
/**
 * Get the default named graph IRI for push/pull operations.
 *
 * Set via [default_endpoint] named_graph in processors.toml.
 * Useful for setting a well-known IRI such as Jena's
 * <urn:x-arq:DefaultGraph> as the default push/pull target,
 * without requiring graph.named_graph in every DataBook frontmatter.
 *
 * Priority (in push/pull):
 *   1. --graph flag                   per-invocation override
 *   2. graph.named_graph frontmatter  per-document declaration
 *   3. default_endpoint.named_graph   per-environment (this function)
 *   4. {id}#{block.id}                fragment-addressing fallback
 *   5. null → GSP ?default            bare fallback
 *
 * @returns {string|null}
 */
export function getDefaultNamedGraph() {
  const config = loadProcessorsConfig();
  return config.default_endpoint?.named_graph ?? null;
}
